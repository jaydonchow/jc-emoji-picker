...
function useLongHover() {
  ...
  const startLongHover = ({event, duration, callback, cancelCallback}) => {
    isLongHover = false; // 重置状态
    ...
    const cancelHover = (e) => {
      clearTimeout(timer);
      cancelCallback();
      if (!isLongHover) return;
      e.preventDefault();
    };

    touchTarget.addEventListener("mouseleave", cancelHover, { once: true, passive: false });
  };

  return startLongHover;
}

export { useMouseScroll, useLongPress, useLongHover };
